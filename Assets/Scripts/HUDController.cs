...
public class HUDController : MonoBehaviour
{
    ...
    {
        while (transform.GetChild(2).GetChild(1).transform.localScale.x != 0f)
        {
            float scaleX = transform.GetChild(2).GetChild(1).transform.localScale.x;
            if (oldScaleX == 0f)
                oldScaleX = scaleX;

            float newScaleX = Mathf.Clamp(scaleX - oldScaleX / 37.5f, 0f, 7.5f);
            transform.GetChild(2).GetChild(1).transform.localScale = new Vector3(newScaleX, 1.5f, 1f);

            yield return null;
        }

        transform.GetChild(2).GetChild(1).transform.localScale = new Vector3(0f, 1.5f, 1f);
        oldScaleX = 0f;
    }
    ...
    public void UpdateProgressBar()
    {
        ...
        transform.GetChild(2).GetChild(1).transform.localScale = new Vector3(destinationX, 1.5f, 1f);
    }
